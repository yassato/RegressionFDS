...
runGWAS = function(type=c("NFDS","PFDS","OD","STVS"), setting=c("split","continuous"), iter=30) {
  for(i in 1:iter) {
    ...
    if(setting=="split") {
      ...
      g_nei = nei_coval(g,smap,scale=20,grouping=group) # set distance ("scale") longer than max. distance of "smap"
      ...
      g_nei = nei_coval(g,smap,scale=1.42) # scale = 1.42 refers only to neighboring individuals
