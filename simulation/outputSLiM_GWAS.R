...
runGWAS = function(type=c("NFDS","PFDS","OD","STVS"), setting=c("animal","plant"), iter=30) {
  for(i in 1:iter) {
    ...
    print(results$AUC)
    saveRDS(results,file=paste0("output/", f_name, "_", setting, ".rds"))
  }
  return(message("Done. Output files are in ./output"))
}

runGWAS(type="NFDS",setting="plant",iter=30)
runGWAS(type="NFDS",setting="animal",iter=30)

runGWAS(type="PFDS",setting="plant",iter=30)
runGWAS(type="PFDS",setting="animal",iter=30)

runGWAS(type="OD",setting="plant",iter=30)
runGWAS(type="OD",setting="animal",iter=30)

runGWAS(type="STVS",setting="plant",iter=30)
runGWAS(type="STVS",setting="animal",iter=30)
